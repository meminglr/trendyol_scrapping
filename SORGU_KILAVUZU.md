...
- **Arama** için **`GET /v1/search?q=...`**, **kategori** için **`GET /v1/category?url=...`** (Trendyol adresi mutlaka **URL encode** edilmeli) kullanılabilir. İsterseniz ikisi de **POST + JSON** ile de çağrılabilir.
- **GET ile linkten** kullanabileceğiniz uçlar: **`/health`**, **`/v1/search`**, **`/v1/category`**, **`/v1/product`**.
...
### 3) Arama / kategori: link veya POST

- **Arama:** `GET /v1/search?q=...` ile link yeterli (yukarıdaki bölüm).
- **Kategori:** `GET /v1/category?url=ENCODE...` ile link mümkün; `url` uzun ve `?` içeriyorsa **POST** veya `curl -G --data-urlencode` daha kolaydır.

**POST** kullanmak isterseniz:

| Yöntem | Ne yaparsınız? |
|--------|----------------|
| **Swagger** | `/docs` → **POST /v1/search** veya **POST /v1/category** → **Try it out** |
| **curl** | Kılavuzdaki `curl -X POST ... -d '{...}'` örnekleri |
| **Kod** | `fetch(..., { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({...}) })` |
...
## 2. Kategori (`POST /v1/category` ve `GET /v1/category`)

Verilen Trendyol kategori veya liste URL’sinden ürün listesi çeker.

### Link ile kategori (`GET /v1/category`)

Parametre **`url`**: Tarayıcıda adres çubuğuna yazarken Trendyol linkinin **tamamını** yüzde kodlamalı (`encode`) olarak verin (`https` → `https%3A%2F%2F`, `/` → `%2F`, `?` → `%3F`, `&` → `%26`). En pratik yol: [Swagger `/docs`](https://trendyolapi.emniva.com/docs) içinde **GET /v1/category** ile denemek veya aşağıdaki `curl` örneği.

| Parametre | Zorunlu | Varsayılan | Açıklama |
|-----------|---------|------------|----------|
| `url` | evet | — | Encode edilmiş tam Trendyol kategori/liste URL’si |
| `pages` | hayır | `5` | 1–50 |
| `page` | hayır | — | Tek sayfa modu |
| `min_price` / `max_price` | hayır | — | Fiyat filtresi |
| `sort` | hayır | `BEST_SELLER` | Diğer uçlarla aynı |

**Örnek (kısa kategori yolu — path’te `?` yok):**

```text
https://trendyolapi.emniva.com/v1/category?url=https%3A%2F%2Fwww.trendyol.com%2Ftuvalet-koku-giderici-x-c105573&pages=1
```

**Terminalde encode’u otomatik yapmak:**

```bash
curl -sS -G 'https://trendyolapi.emniva.com/v1/category' \
  --data-urlencode 'url=https://www.trendyol.com/sr?qt=kulaklik&os=1' \
  --data-urlencode 'pages=2'
```

`TRENDYOL_API_KEY` aktifse tarayıcı linkinde header olmadığı için **401** alırsınız; `curl` ile `X-API-Key` veya `/docs` kullanın.

### İstek gövdesi (JSON) — `POST`
