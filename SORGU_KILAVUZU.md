...
- Bu yüzden **arama** (`/v1/search`) ve **kategori** (`/v1/category`) için “tek bir tıklanabilir link” üretmek genelde mümkün değildir; bunlar **POST + JSON** ister.
- **GET ile linkten** kullanabileceğiniz uçlar: **`/health`** ve isteğe bağlı **`/v1/product`** (aşağıda).
...
## 1. Arama (`POST /v1/search`)

Trendyol’da kelime ile arama yapar; ürün listesi döner.
